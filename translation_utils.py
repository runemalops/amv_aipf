...
def get_translated(obj, field, lang='en'):
    """
    Get translated value for a field with auto-translate fallback.
    ...
    cache_key = f"{obj.__class__.__name__}_{obj.id}_{field}_{lang}"
    ...
    if original_value:
        translated = translate_text(original_value, target_lang=lang)
        # Cache the result
        _translations_cache[cache_key] = translated
        return translated
    ...
    for item in items:
        ...
        for field in fields:
            if field == 'technologies' or field == 'responsibilities':
                ...
                if raw_value:
                    item_dict[field] = raw_value.split(',' if field == 'technologies' else '\n')
                else:
                    item_dict[field] = []
            else:
